...
const CanvasParser = {
  ...
  getTeacherInstructions() {
    const specificSelector = 'span.user_content.enhanced[data-resource-type="discussion_topic.body"]';
  ...
  getStudentPosts() {
    ...
    entryElements.forEach((element, index) => {
      const authorNameElement = element.querySelector('[data-testid="author_name"]');
      ...
      const contentElement = element.querySelector('.userMessage .user_content.enhanced[data-resource-type="discussion_topic.reply"]');
      let content = '';
      ...
      if (content && content.length > 10) {
        ...
        const isMainTopic = element.querySelector('[data-resource-type="discussion_topic.body"]');
        if (!isMainTopic) {
          posts.push({
            author,
            content,
